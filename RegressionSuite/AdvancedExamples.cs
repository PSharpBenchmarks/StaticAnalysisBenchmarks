...
namespace AdvancedExamples
{
    ...
    internal class O3 : Machine
    {
        ...
        private class Init : State
        {
            protected override void OnEntry()
            {
                var machine = this.Machine as O3;

                machine.Target = Machine.Factory.CreateMachine<O3>();
                var e = new Envelope("London", 0);

                try
                {
                    this.Send(machine.Target, new eUnit(e));
                    e = new Envelope();
                }
                catch
                {
